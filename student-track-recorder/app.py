...
@app.route('/main_credentials', methods=['GET', 'PUT'])
def main_credentials():
    ...
    creds = load_main_credentials()
    # If changing password, verify current first against plain or hash
    if new_password:
        ok = False
        if creds.get('password') is not None:
            ok = (current_password and current_password == str(creds.get('password')))
        else:
            ok = (current_password and verify_main_password(creds.get('password_hash', ''), current_password))
        if not ok:
            return jsonify({'success': False, 'error': 'Current password is incorrect'}), 400
        # store plain password; backend will no longer rewrite to hash
        creds['password'] = new_password
        if 'password_hash' in creds:
            creds.pop('password_hash', None)
    if new_username:
        creds['username'] = new_username

    save_main_credentials(creds)
