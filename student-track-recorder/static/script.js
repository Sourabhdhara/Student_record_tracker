...
class StudentTrackRecorder {
    ...
    async ensureUserRole() {
        if (this.currentUser && this.currentUser.type) return this.currentUser;
        try {
            const who = await this.apiCall('/whoami');
            const t = who && who.userType;
            if (t) {
                this.currentUser = { type: t, userId: (who.userId || this.currentUser?.userId) };
                // reflect role-specific UI
                const secTabBtn = document.getElementById('secondaryAdminTabBtn');
                const addSecBtn = document.getElementById('addSecondaryAdminBtn');
                if (secTabBtn) secTabBtn.style.display = (t === 'faculty') ? '' : 'none';
                if (addSecBtn) addSecBtn.style.display = (t === 'faculty') ? '' : 'none';
            }
        } catch (e) {}
    ...
    setupEventListeners() {
        ...
        // Toggle forgot/reset button for faculty login based on account type.
        const facultyAccountType = document.getElementById('facultyAccountType');
        if (facultyAccountType) {
            facultyAccountType.addEventListener('change', () => this.updateFacultyForgotButtonVisibility());
        }
        ...
        this.updateFacultyForgotButtonVisibility();
    }

    updateFacultyForgotButtonVisibility() {
        const forgotBtn = document.getElementById('facultyForgotBtn');
        const accountType = document.getElementById('facultyAccountType');
        if (!forgotBtn || !accountType) return;
        forgotBtn.style.display = accountType.value === 'secondary' ? 'block' : 'none';
    }
    ...
    setupModalForms() {
        ...
        if (notesUploadBtn) notesUploadBtn.onclick = () => this.handleUploadNote();
        const notesAddSubjectBtn = document.getElementById('notesAddSubjectBtn');
        if (notesAddSubjectBtn) notesAddSubjectBtn.onclick = () => this.handleAddNotesSubject();
    }
    ...
    showFacultyDashboard(noPush = false) {
        ...
        const secTabBtn = document.getElementById('secondaryAdminTabBtn');
        const addSecBtn = document.getElementById('addSecondaryAdminBtn');
        const accountBtn = document.getElementById('facultyAccountBtn');
        ...
        if (accountBtn) accountBtn.style.display = (this.currentUser?.type === 'secondary') ? '' : 'none';
    ...
    async requestPasswordResetOtp(role, identity) {
        const payload = { role, ...identity };
        const res = await this.apiCall('/otp/request_password_reset', {
            method: 'POST',
    ...
    async verifyPasswordResetOtp(resetId, otp, newPassword) {
        return await this.apiCall('/otp/verify_password_reset', {
            method: 'POST',
            body: JSON.stringify({ resetId, otp, newPassword })
        });
    }

    openFacultyForgotPassword() {
        let modal = document.getElementById('facultyForgotPasswordModal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'facultyForgotPasswordModal';
            modal.className = 'modal';
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 520px;">
                    <div class="modal-header">
                        <h3>Faculty Forgot / Reset Password</h3>
                        <button class="close-btn" onclick="closeModal('facultyForgotPasswordModal')">&times;</button>
                    </div>
                    <form id="facultyForgotPasswordForm" style="padding:16px;">
                        <div class="form-group">
                            <label for="forgotFacultyUserId">Secondary Admin User ID</label>
                            <input type="text" id="forgotFacultyUserId" required>
                        </div>
                        <div class="form-group" id="forgotFacultyEmailWrap">
                            <label for="forgotFacultyEmail">Secondary Admin Email</label>
                            <input type="email" id="forgotFacultyEmail">
                        </div>
                        <div class="form-group">
                            <label for="forgotFacultyOtp">OTP</label>
                            <input type="text" id="forgotFacultyOtp" maxlength="6" placeholder="Enter 6-digit OTP" required>
                        </div>
                        <div class="form-group">
                            <label for="forgotFacultyNewPassword">New Password</label>
                            <input type="password" id="forgotFacultyNewPassword" required>
                        </div>
                        <div class="form-actions" style="justify-content:flex-end;">
                            <button type="button" class="cancel-btn" id="requestFacultyOtpBtn">Request OTP</button>
                            <button type="button" class="cancel-btn" onclick="closeModal('facultyForgotPasswordModal')">Cancel</button>
                            <button type="submit" class="submit-btn">Verify OTP & Reset</button>
                        </div>
                    </form>
                </div>
            `;
            document.body.appendChild(modal);
            modal.addEventListener('click', (e) => { if (e.target === modal) this.closeModal('facultyForgotPasswordModal'); });
        }

        const accountType = document.getElementById('facultyAccountType')?.value || 'main';
        const userId = document.getElementById('facultyUserId')?.value || '';
        const userEl = document.getElementById('forgotFacultyUserId');
        if (userEl) userEl.value = userId;
        if (accountType !== 'secondary') {
            this.showError('facultyLoginError', 'Forgot/Reset is available only for secondary admin.');
            return;
        }

        const reqBtn = document.getElementById('requestFacultyOtpBtn');
        if (reqBtn) reqBtn.onclick = () => this.requestFacultyForgotOtp();
        const form = document.getElementById('facultyForgotPasswordForm');
        if (form) form.onsubmit = (e) => this.handleFacultyForgotPassword(e);
        this.showModal('facultyForgotPasswordModal');
    }

    async requestFacultyForgotOtp() {
        const userId = (document.getElementById('forgotFacultyUserId')?.value || '').trim();
        const email = (document.getElementById('forgotFacultyEmail')?.value || '').trim();
        if (!userId || !email) {
            alert('User ID and email are required to request OTP.');
            return;
        }
        try {
            const resetId = await this.requestPasswordResetOtp('secondary', { userId, email });
            if (resetId) {
                this._facultyForgotResetId = resetId;
                this.showSuccessDialog('✓ OTP sent to your email. It expires in about 5 minutes.');
            }
        } catch (err) {}
    }

    async handleFacultyForgotPassword(e) {
        e.preventDefault();
        const userId = (document.getElementById('forgotFacultyUserId')?.value || '').trim();
        const email = (document.getElementById('forgotFacultyEmail')?.value || '').trim();
        const newPassword = (document.getElementById('forgotFacultyNewPassword')?.value || '').trim();

        if (!userId || !newPassword) {
            alert('User ID and new password are required.');
            return;
        }
        if (!email) {
            alert('Email is required for secondary admin reset.');
            return;
        }
        const otp = (document.getElementById('forgotFacultyOtp')?.value || '').trim();
        if (!otp) {
            alert('Enter OTP to continue.');
            return;
        }
        if (!this._facultyForgotResetId) {
            alert('Please request OTP first.');
            return;
        }

        try {
            const res = await this.verifyPasswordResetOtp(this._facultyForgotResetId, otp, newPassword);
            if (res && res.success) {
                this.closeModal('facultyForgotPasswordModal');
                this._facultyForgotResetId = null;
                this.showSuccessDialog('Password Reset Successfully!\n\nClick OK to go back to login.', () => this.showLandingPage());
            }
        } catch (err) {}
    }

    openStudentForgotPassword() {
        let modal = document.getElementById('studentForgotPasswordModal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'studentForgotPasswordModal';
            modal.className = 'modal';
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 520px;">
                    <div class="modal-header">
                        <h3>Student Forgot / Reset Password</h3>
                        <button class="close-btn" onclick="closeModal('studentForgotPasswordModal')">&times;</button>
                    </div>
                    <form id="studentForgotPasswordForm" style="padding:16px;">
                        <div class="form-group">
                            <label for="forgotStudentRoll">Roll Number</label>
                            <input type="text" id="forgotStudentRoll" required>
                        </div>
                        <div class="form-group">
                            <label for="forgotStudentEmail">Email</label>
                            <input type="email" id="forgotStudentEmail" required>
                        </div>
                        <div class="form-group">
                            <label for="forgotStudentOtp">OTP</label>
                            <input type="text" id="forgotStudentOtp" maxlength="6" placeholder="Enter 6-digit OTP" required>
                        </div>
                        <div class="form-group">
                            <label for="forgotStudentNewPassword">New Password</label>
                            <input type="password" id="forgotStudentNewPassword" required>
                        </div>
                        <div class="form-actions" style="justify-content:flex-end;">
                            <button type="button" class="cancel-btn" id="requestStudentOtpBtn">Request OTP</button>
                            <button type="button" class="cancel-btn" onclick="closeModal('studentForgotPasswordModal')">Cancel</button>
                            <button type="submit" class="submit-btn">Verify OTP & Reset</button>
                        </div>
                    </form>
                </div>
            `;
            document.body.appendChild(modal);
            modal.addEventListener('click', (e) => { if (e.target === modal) this.closeModal('studentForgotPasswordModal'); });
        }

        const roll = document.getElementById('studentRoll')?.value || '';
        const email = document.getElementById('studentEmail')?.value || '';
        const rollEl = document.getElementById('forgotStudentRoll');
        const emailEl = document.getElementById('forgotStudentEmail');
        if (rollEl) rollEl.value = roll;
        if (emailEl) emailEl.value = email;

        const reqBtn = document.getElementById('requestStudentOtpBtn');
        if (reqBtn) reqBtn.onclick = () => this.requestStudentForgotOtp();
        const form = document.getElementById('studentForgotPasswordForm');
        if (form) form.onsubmit = (e) => this.handleStudentForgotPassword(e);
        this.showModal('studentForgotPasswordModal');
    }

    async requestStudentForgotOtp() {
        const rollNumber = (document.getElementById('forgotStudentRoll')?.value || '').trim();
        const email = (document.getElementById('forgotStudentEmail')?.value || '').trim();
        if (!rollNumber || !email) {
            alert('Roll number and email are required to request OTP.');
            return;
        }
        try {
            const resetId = await this.requestPasswordResetOtp('student', { rollNumber, email });
            if (resetId) {
                this._studentForgotResetId = resetId;
                this.showSuccessDialog('✓ OTP sent to your email. It expires in about 5 minutes.');
            }
        } catch (err) {}
    }

    async handleStudentForgotPassword(e) {
        e.preventDefault();
        const rollNumber = (document.getElementById('forgotStudentRoll')?.value || '').trim();
        const email = (document.getElementById('forgotStudentEmail')?.value || '').trim();
        const newPassword = (document.getElementById('forgotStudentNewPassword')?.value || '').trim();

        if (!rollNumber || !email || !newPassword) {
            alert('Roll number, email and new password are required.');
            return;
        }
        const otp = (document.getElementById('forgotStudentOtp')?.value || '').trim();
        if (!otp) {
            alert('Enter OTP to continue.');
            return;
        }
        if (!this._studentForgotResetId) {
            alert('Please request OTP first.');
            return;
        }

        try {
            const res = await this.verifyPasswordResetOtp(this._studentForgotResetId, otp, newPassword);
            if (res && res.success) {
                this.closeModal('studentForgotPasswordModal');
                this._studentForgotResetId = null;
                this.showSuccessDialog('Password Reset Successfully!\n\nClick OK to go back to login.', () => this.showLandingPage());
            }
        } catch (err) {}
    }

    openFacultyAccountModal() {
        if (!(this.currentUser && this.currentUser.type === 'secondary')) return;
        let modal = document.getElementById('facultyAccountModal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'facultyAccountModal';
            modal.className = 'modal';
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 620px;">
                    <div class="modal-header">
                        <h3>Account Settings</h3>
                        <button class="close-btn" onclick="closeModal('facultyAccountModal')">&times;</button>
                    </div>
                    <div style="padding:16px; display:grid; gap:16px;">
                        <form id="facultyAccountChangeForm">
                            <h4 style="margin-bottom:8px;">Change Password</h4>
                            <div class="form-group">
                                <label for="facultyAccCurrentPw">Current Password</label>
                                <input type="password" id="facultyAccCurrentPw" required>
                            </div>
                            <div class="form-group">
                                <label for="facultyAccNewPw">New Password</label>
                                <input type="password" id="facultyAccNewPw" required>
                            </div>
                            <div class="form-actions" style="justify-content:flex-end;">
                        ...
                        <hr>
                        <form id="facultyAccountForgotForm">
                            <h4 style="margin-bottom:8px;">Forgot / Reset Password</h4>
                            <div class="form-group">
                                <label for="facultyAccUserId">User ID</label>
                                <input type="text" id="facultyAccUserId" required>
                            </div>
                            <div class="form-group" id="facultyAccEmailWrap" style="display:none;">
                                <label for="facultyAccEmail">Email</label>
                                <input type="email" id="facultyAccEmail">
                            </div>
                            <div class="form-group">
                                <label for="facultyAccOtp">OTP</label>
                                <input type="text" id="facultyAccOtp" maxlength="6" placeholder="Enter 6-digit OTP" required>
                            </div>
                            <div class="form-group">
                                <label for="facultyAccForgotNewPw">New Password</label>
                                <input type="password" id="facultyAccForgotNewPw" required>
                            </div>
                            <div class="form-actions" style="justify-content:flex-end;">
                                <button type="button" class="cancel-btn" id="facultyAccRequestOtpBtn">Request OTP</button>
                                <button type="submit" class="submit-btn">Verify OTP & Reset</button>
                            </div>
                        </form>
                    </div>
            ...
            `;
            document.body.appendChild(modal);
            modal.addEventListener('click', (e) => { if (e.target === modal) this.closeModal('facultyAccountModal'); });
        }

        const userIdEl = document.getElementById('facultyAccUserId');
        const emailWrap = document.getElementById('facultyAccEmailWrap');
        if (userIdEl) userIdEl.value = this.currentUser?.userId || '';
        if (emailWrap) emailWrap.style.display = this.currentUser?.type === 'secondary' ? '' : 'none';

        const changeForm = document.getElementById('facultyAccountChangeForm');
        if (changeForm) changeForm.onsubmit = (e) => this.handleFacultyAccountChangePassword(e);
        const forgotForm = document.getElementById('facultyAccountForgotForm');
        if (forgotForm) forgotForm.onsubmit = (e) => this.handleFacultyAccountForgotPassword(e);
        const otpBtn = document.getElementById('facultyAccRequestOtpBtn');
        if (otpBtn) otpBtn.onclick = () => this.requestFacultyAccountForgotOtp();

        this.showModal('facultyAccountModal');
    }

    async requestFacultyAccountForgotOtp() {
        const userId = (document.getElementById('facultyAccUserId')?.value || '').trim();
        const email = (document.getElementById('facultyAccEmail')?.value || '').trim();
        if (!userId || !email) {
            alert('User ID and email are required to request OTP.');
            return;
        }
        try {
            const resetId = await this.requestPasswordResetOtp('secondary', { userId, email });
            if (resetId) {
                this._facultyPanelForgotResetId = resetId;
                this.showSuccessDialog('✓ OTP sent to your email. It expires in about 5 minutes.');
            }
        } catch (err) {}
    }

    async handleFacultyAccountChangePassword(e) {
        e.preventDefault();
        const currentPassword = (document.getElementById('facultyAccCurrentPw')?.value || '').trim();
        const newPassword = (document.getElementById('facultyAccNewPw')?.value || '').trim();
        if (!currentPassword || !newPassword) {
            alert('Current and new password are required.');
            return;
        }
        try {
            const res = await this.apiCall('/secondary_self_update_password', {
                method: 'POST',
                body: JSON.stringify({ currentPassword, newPassword })
            });
            if (res && res.success) {
                this.showSuccessDialog('Password Changed Successfully!');
                setTimeout(() => this.closeModal('facultyAccountModal'), 500);
            }
        } catch (err) {}
    }

    async handleFacultyAccountForgotPassword(e) {
        e.preventDefault();
        const userId = (document.getElementById('facultyAccUserId')?.value || '').trim();
        const email = (document.getElementById('facultyAccEmail')?.value || '').trim();
        const otp = (document.getElementById('facultyAccOtp')?.value || '').trim();
        const newPassword = (document.getElementById('facultyAccForgotNewPw')?.value || '').trim();
        if (!userId || !newPassword) {
            alert('User ID and new password are required.');
            return;
        }
        if (this.currentUser?.type === 'secondary' && !email) {
            alert('Email is required for secondary admin reset.');
            return;
        }
        if (!otp) {
        ...
        if (!this._facultyPanelForgotResetId) {
            alert('Please request OTP first.');
            return;
        }

        try {
            const res = await this.verifyPasswordResetOtp(this._facultyPanelForgotResetId, otp, newPassword);
            if (res && res.success) {
                this._facultyPanelForgotResetId = null;
                this.showSuccessDialog('Password Reset Successfully!');
                setTimeout(() => this.closeModal('facultyAccountModal'), 500);
            }
        } catch (err) {}
    ...
    async renderNotesTab() {
        // ensure we know current role (after page refresh)
        await this.ensureUserRole();
        // show subject options based on role
        const sel = document.getElementById('notesSubjectSelect');
        const addSubBtn = document.getElementById('notesAddSubjectBtn');
        const addSubInput = document.getElementById('notesAddSubjectInput');
        if (!this.currentCourse || !this.currentYear || !this.currentSection) { sel.innerHTML = '<option value="">Select section first</option>'; return; }
        try {
            // Determine if user is main admin by presence of secondaryAdminTab button visibility
            const isMain = (this.currentUser?.type === 'faculty');
            if (isMain) {
                addSubBtn.style.display = '';
                addSubInput.style.display = '';
            } else {
                addSubBtn.style.display = 'none';
                addSubInput.style.display = 'none';
            }
            const subjects = await this.apiCall(`/attendance/subjects/${encodeURIComponent(this.currentCourse)}/${encodeURIComponent(this.currentYear)}/${encodeURIComponent(this.currentSection)}`);
            sel.innerHTML = '';
            ...
            // auto list notes for first subject
            if (subjects && subjects[0]) this.renderNotesList(subjects[0]);
            sel.onchange = () => this.renderNotesList(sel.value);
        } catch (e) {
            sel.innerHTML = '<option value="">Failed to load</option>';
        }
    }

    async handleAddNotesSubject() {
        const input = document.getElementById('notesAddSubjectInput');
        const name = (input?.value || '').trim();
        if (!name) { alert('Enter subject name'); return; }
        try {
            const res = await this.apiCall(`/attendance/subjects/${encodeURIComponent(this.currentCourse)}/${encodeURIComponent(this.currentYear)}/${encodeURIComponent(this.currentSection)}`, {
                method: 'POST', body: JSON.stringify({ name })
            });
            if (res && res.success) {
                input.value = '';
                this.showSuccess('Subject added.');
    ...
    async handleUploadNote() {
        ...
        fd.append('file', file);
        try {
            const res = await this.apiCall(`/notes/${encodeURIComponent(this.currentCourse)}/${encodeURIComponent(this.currentYear)}/${encodeURIComponent(this.currentSection)}`, { method: 'POST', body: fd, headers: {} });
            if (res && res.success) {
                this.showSuccess('Note uploaded.');
                // refresh list
                await this.renderNotesList(subj);
                if (fileInput) fileInput.value = '';
                if (document.getElementById('notesTitle')) document.getElementById('notesTitle').value = '';
                if (document.getElementById('notesDesc')) document.getElementById('notesDesc').value = '';
            }
        } catch (e) {}
    }

    async deleteNote(noteId, subject) {
        if (!confirm('Delete this note?')) return;
        try {
            const res = await this.apiCall(`/notes/${encodeURIComponent(this.currentCourse)}/${encodeURIComponent(this.currentYear)}/${encodeURIComponent(this.currentSection)}/${encodeURIComponent(noteId)}`, { method: 'DELETE' });
            if (res && res.success) {
                this.showSuccess('Note deleted.');
                await this.renderNotesList(subject);
            } else {
                this.showError('Delete Note Error', (res && res.error) || 'Failed to delete.');
            }
        } catch (e) {}
    }

    async renderNotesList(subject) {
        const list = document.getElementById('notesList');
        if (!list) return;
        list.innerHTML = '<div class="item"><p class="muted">Loading...</p></div>';
        try {
            const items = await this.apiCall(`/notes/${encodeURIComponent(this.currentCourse)}/${encodeURIComponent(this.currentYear)}/${encodeURIComponent(this.currentSection)}?subject=${encodeURIComponent(subject)}`);
            list.innerHTML = '';
            if (!items || !items.length) { list.innerHTML = '<div class="item"><p class="muted">No notes yet.</p></div>'; return; }
            items.slice().sort((a,b)=> (a.uploadedAt||'').localeCompare(b.uploadedAt||''))
                .forEach(n => {
                    const div = document.createElement('div');
                    div.className = 'item';
                    const uploaded = n.uploadedAt ? new Date(n.uploadedAt).toLocaleString() : '';
                    const canDelete = (this.currentUser?.type === 'faculty' || this.currentUser?.type === 'secondary');
                    const actions = canDelete
                        ? `<button class="delete-item-btn" style="width:auto;min-width:120px;" onclick="app.deleteNote('${n.id}', '${subject}')">Delete</button> <a class="add-btn" style="width:auto;min-width:120px;" href="${n.file?.url}" target="_blank">Download</a>`
                        : `<a class="add-btn" style="width:auto;min-width:120px;" href="${n.file?.url}" target="_blank">Download</a>`;
                    div.innerHTML = `
                        <div class="item-info">
                            <h4>${this.escapeHtml(n.title || n.file?.filename || '')}</h4>
                            ${n.description ? `<p>${this.escapeHtml(n.description)}</p>`: ''}
                            <p class="muted"><small>By ${this.escapeHtml(n.uploadedBy?.name || '')} • ${uploaded}</small></p>
                        </div>
                        <div class="item-actions">
                            ${actions}
                        </div>
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 760px;">
                    <div class="modal-header">
                        <h3>Notes</h3>
                        <button class="close-btn" onclick="closeModal('studentNotesModal')">&times;</button>
                    </div>
                    <div style="padding: 16px;">
                        <div class="form-group">
                            <label for="studentNotesSubject">Select Subject</label>
                            <select id="studentNotesSubject"></select>
                        </div>
                        <div id="studentNotesList" class="items-list" style="max-height:60vh;overflow:auto;"></div>
                    </div>
                </div>`;
            document.body.appendChild(modal);
            modal.addEventListener('click', (e) => { if (e.target === modal) this.closeModal('studentNotesModal'); });
        }
        this.showModal('studentNotesModal');
        this.renderStudentNotesUI();
    }

    async renderStudentNotesUI() {
        const sel = document.getElementById('studentNotesSubject');
        ...
        try {
            ...
            if (!subjects || !subjects.length) {
                sel.innerHTML = '<option value="">No subjects</option>';
                list.innerHTML = '<div class="item"><p class="muted">No subjects yet.</p></div>';
                return;
            }
            subjects.forEach(s => { const o=document.createElement('option'); o.value=s; o.textContent=s; sel.appendChild(o); });
            sel.onchange = () => this.renderStudentNotesList(sel.value);
    ...
    async renderStudentNotesList(subject) {
        const list = document.getElementById('studentNotesList');
        if (!list) return;
        list.innerHTML = '<div class="item"><p class="muted">Loading...</p></div>';
        try {
            const items = await this.apiCall(`/student_notes?subject=${encodeURIComponent(subject)}`);
            list.innerHTML = '';
            if (!items || !items.length) { list.innerHTML = '<div class="item"><p class="muted">No notes yet.</p></div>'; return; }
            items.slice().sort((a,b)=> (a.uploadedAt||'').localeCompare(b.uploadedAt||''))
                .forEach(n => {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="item-actions">
                            <a class="add-btn" href="${n.file?.url}" target="_blank">Download</a>
                        </div>`;
                    list.appendChild(div);
                });
        } catch (e) {
    ...
    openStudentEditProfile() {
            ...
            modal.innerHTML = `
                <div class="modal-content" style="max-width: 520px;">
                    ...
                    <form id="studentEditProfileForm" style="padding:16px;">
                        ...
                        <h4 style="margin-bottom:8px;">Forgot / Reset Password</h4>
                        <div class="form-group">
                            <label for="studentForgotRollPanel">Roll Number</label>
                            <input type="text" id="studentForgotRollPanel" placeholder="Enter roll number">
                        </div>
                        <div class="form-group">
                            <label for="studentForgotEmailPanel">Email</label>
                            <input type="email" id="studentForgotEmailPanel" placeholder="Enter email">
                        </div>
                        <div class="form-group">
                            <label for="studentForgotOtpPanel">OTP</label>
                            <input type="text" id="studentForgotOtpPanel" maxlength="6" placeholder="Enter 6-digit OTP">
                        </div>
                        <div class="form-group">
                            <label for="studentForgotNewPwPanel">New Password</label>
                            <input type="password" id="studentForgotNewPwPanel" placeholder="Enter new password">
                        </div>
                        <div class="form-actions" style="justify-content:flex-end;">
                            <button type="button" class="cancel-btn" id="studentForgotRequestOtpBtn">Request OTP</button>
                            <button type="button" class="submit-btn" id="studentForgotResetBtn">Reset Password</button>
                        </div>
                    </form>
                </div>
            `;
        ...
        const requestBtn = document.getElementById('studentForgotRequestOtpBtn');
        if (requestBtn) requestBtn.onclick = () => this.requestStudentForgotOtpFromPanel();
        const forgotBtn = document.getElementById('studentForgotResetBtn');
        if (forgotBtn) forgotBtn.onclick = () => this.handleStudentForgotResetFromPanel();
        this.showModal('studentEditProfileModal');
        this.prefillStudentResetIdentity();
    }

    async prefillStudentResetIdentity() {
        try {
            const data = await this.apiCall('/student_data');
            const student = data?.student || {};
            const rollEl = document.getElementById('studentForgotRollPanel');
            const emailEl = document.getElementById('studentForgotEmailPanel');
            if (rollEl) rollEl.value = student.rollNumber || '';
            if (emailEl) emailEl.value = student.email || '';
        } catch (err) {}
    }

    async handleStudentForgotResetFromPanel() {
        const rollNumber = (document.getElementById('studentForgotRollPanel')?.value || '').trim();
        const email = (document.getElementById('studentForgotEmailPanel')?.value || '').trim();
        const otp = (document.getElementById('studentForgotOtpPanel')?.value || '').trim();
        const newPassword = (document.getElementById('studentForgotNewPwPanel')?.value || '').trim();
        if (!rollNumber || !email || !newPassword) {
            alert('Roll number, email and new password are required.');
            return;
        }
        ...
        if (!this._studentPanelForgotResetId) {
            alert('Please request OTP first.');
            return;
        }
        try {
            const res = await this.verifyPasswordResetOtp(this._studentPanelForgotResetId, otp, newPassword);
            if (res && res.success) {
                this._studentPanelForgotResetId = null;
                this.showSuccessDialog('Password Reset Successfully!');
                const np = document.getElementById('studentForgotNewPwPanel');
                if (np) np.value = '';
            }
        } catch (err) {}
    }

    async requestStudentForgotOtpFromPanel() {
        const rollNumber = (document.getElementById('studentForgotRollPanel')?.value || '').trim();
        const email = (document.getElementById('studentForgotEmailPanel')?.value || '').trim();
        if (!rollNumber || !email) {
            alert('Roll number and email are required to request OTP.');
        ...
        try {
            const resetId = await this.requestPasswordResetOtp('student', { rollNumber, email });
            if (resetId) {
                this._studentPanelForgotResetId = resetId;
                this.showSuccessDialog('✓ OTP sent to your email. It expires in about 5 minutes.');
            }
        } catch (err) {}
    }

    openStudentPasswordPanel() {
        this.openStudentEditProfile();
    }
    ...
    async handleStudentSelfUpdate(e) {
        ...
        try {
            ...
            if (res && res.success) {
                this.showSuccessDialog('Profile Updated Successfully!');
                setTimeout(() => this.closeModal('studentEditProfileModal'), 500);
    ...
    renderStudentAttendanceContainer() {
        ...
        <button class="add-btn" onclick="app.openStudentNotes()">Notes</button>
        </div>
        `;
        }
    ...
    showSuccessDialog(message, onOk = null) {
        const dialogId = 'successDialog_' + Date.now();
        ...
        if (!dialog) {
            ...
            dialog.innerHTML = `
                <div class="modal-content" style="max-width: 400px;">
                    ...
                    <div class="form-actions" style="justify-content: center; padding: 20px;">
                        <button class="submit-btn" id="okBtn_${dialogId}" style="min-width: 100px;">OK</button>
                    </div>
                </div>
            `;
            ...
            const okBtn = document.getElementById('okBtn_' + dialogId);
            if (okBtn) {
                okBtn.onclick = () => {
                    this.closeModal(dialogId);
                    document.body.removeChild(dialog);
    ...
    showError(elementId, message) {
        ...
        if (errorElement) {
            ...
            // Fallback: show error as popup instead of alert
            this.showErrorDialog(message);
        }
    }

    showErrorDialog(message) {
        const dialogId = 'errorDialog_' + Date.now();
        ...
        dialog.innerHTML = `
            <div class="modal-content" style="max-width: 400px;">
                ...
                <div style="padding: 20px; text-align: center;">
                    <p style="font-size: 16px; margin: 20px 0; color: #d32f2f;">${message}</p>
                </div>
                <div class="form-actions" style="justify-content: center; padding: 20px;">
                    <button class="cancel-btn" id="okBtn_${dialogId}" style="min-width: 100px;">OK</button>
                </div>
            </div>
        `;
        document.body.appendChild(dialog);
...
function openStudentForgotPassword() { app.openStudentForgotPassword(); }
function openFacultyForgotPassword() { app.openFacultyForgotPassword(); }
function openFacultyAccountModal() { app.openFacultyAccountModal(); }
function openStudentPasswordPanel() { app.openStudentPasswordPanel(); }
